...
public final class Util {
  ...
  /**
   * Compute the bitwise AND between two long arrays and write the set bits in the mask.
   *
   * @param mask where we write
   * @param bitmap1 first bitmap
   * @param bitmap2 second bitmap
   */
  public static void fillArrayAND(final short[] mask, final long[] bitmap1,
                                  final long[] bitmap2) {
    int pos = 0;
    ...
    for (int k = 0; k < bitmap1.length; ++k) {
      ...
      while (bitset != 0) {
        ...
        mask[pos++] = (short) (k * 64 + Long.bitCount(t - 1));
        bitset ^= t;
      }
    }
  }
  ...
  /**
   * Compute the bitwise ANDNOT between two long arrays and write the set bits in the mask.
   *
   * @param mask where we write
   * @param bitmap1 first bitmap
   * @param bitmap2 second bitmap
   */
  public static void fillArrayANDNOT(final short[] mask, final long[] bitmap1,
                                     final long[] bitmap2) {
    int pos = 0;
    if (bitmap1.length != bitmap2.length) {
      throw new IllegalArgumentException("not supported");
  ...
  /**
   * Compute the bitwise XOR between two long arrays and write the set bits in the mask.
   *
   * @param mask where we write
   * @param bitmap1 first bitmap
   * @param bitmap2 second bitmap
   */
  public static void fillArrayXOR(final short[] mask, final long[] bitmap1,
                                  final long[] bitmap2) {
    int pos = 0;
    if (bitmap1.length != bitmap2.length) {
      throw new IllegalArgumentException("not supported");
   ...
   * @return cardinality of the difference
   */
  public static int unsignedDifference(MaskIterator set1, MaskIterator set2,
                                       final short[] buffer) {
    int pos = 0;
    if (!set2.hasNext()) {
      while (set1.hasNext()) {
